...
async fn search_cities(query: &str) -> Result<Vec<String>, String> {
    ...
    match client
        ...
        .header(
            "User-Agent",
            "WhisperWeather App (https://github.com/example/whisper-weather)",
        )
        .send()
        .await
    {
                        ...
                        let cities: Vec<String> = results
                            ...
                            .filter_map(|result| {
                                ...
                                if let Some(address) = result.get("address") {
                                    ...
                                    if let Some(village) =
                                        address.get("village").and_then(|v| v.as_str())
                                    {
...
async fn open_url<'a>(
    url: String,
    app: tauri::AppHandle,
    rate_limit: tauri::State<'a, Arc<Mutex<HashMap<String, u64>>>>,
) -> Result<(), String> {
...
async fn get_user_stats() -> Result<serde_json::Value, String> {
    ...
    let resolved_bugs = bugs
        .iter()
        .filter(|bug| bug.get("status") == Some(&serde_json::Value::String("resolved".to_string())))
        .count();
    let critical_bugs = bugs
        .iter()
        .filter(|bug| {
            bug.get("severity") == Some(&serde_json::Value::String("critical".to_string()))
        })
        .count();
