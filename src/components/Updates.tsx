...
export default function Updates() {
  ...
  const checkForUpdates = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.github.com/repos/GalaxySal/Whisper-Weather/releases",
      );
      ...
      const latestRelease =
        releases.find((release: any) => {
          ...
          if (release.tag_name.includes("dev") || release.name.includes("dev"))
            return false;
      ...
      if (current !== latest && isValidVersion) {
        ...
        toast.info(t("updateAvailable"), {
          description:
            language === "tr"
              ? `Versiyon ${latestRelease.tag_name} yayınlandı`
              : `Version ${latestRelease.tag_name} released`,
          duration: 5000,
        ...
        toast.success(t("upToDate"), {
          description:
            language === "tr"
              ? `Mevcut versiyon: v${current} - Son versiyon`
              : `Current version: v${current} - Latest version`,
          duration: 3000,
          position: "bottom-right",
        });
      }
  ...
  const downloadUpdate = async () => {
    ...
    try {
      ...
      if (!asset) {
        ...
        window.open(
          "https://github.com/GalaxySal/Whisper-Weather/releases",
          "_blank",
        );

        toast.info(
          language === "tr" ? "GitHub sayfası açıldı!" : "GitHub page opened!",
          {
            description:
              language === "tr"
                ? "Tüm versiyonları görmek için releases sayfasını ziyaret edin"
                : "Visit releases page to see all versions",
            duration: 3000,
            position: "bottom-right",
          },
        );
      ...
      if (isTauri) {
        ...
        try {
          ...
          toast.success(
            language === "tr" ? "İndirme başladı!" : "Download started!",
            {
              description:
                language === "tr"
                  ? `${asset.name} indiriliyor...`
                  : `${asset.name} downloading...`,
              duration: 4000,
              position: "bottom-right",
            },
          );
          ...
          toast.error(
            language === "tr" ? "İndirme başarısız" : "Download failed",
            {
              description:
                language === "tr"
                  ? "Lütfen tekrar deneyin veya GitHub releases sayfasını ziyaret edin"
                  : "Please try again or visit GitHub releases",
              duration: 4000,
              position: "bottom-right",
            },
          );
        ...
        toast.info(language === "tr" ? "Web versiyonu" : "Web version", {
          description:
            language === "tr"
              ? "Güncellemeler için GitHub'ı ziyaret edin"
  ...
  const formatChangelog = (body: string) => {
    ...
    return body
      ...
      .replace(
        /###\s*(.+)/g,
        '<h4 class="font-semibold text-base mb-2 mt-3">$1</h4>',
      )
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 text-white border border-white/20">
        ...
        {latestRelease && (
          ...
          >
            ...
            {latestRelease.body && (
              <div className="mb-6">
                <h4 className="font-semibold mb-3">{t("whatsNew")}</h4>
                <div className="bg-black/20 rounded-lg p-4 max-h-40 overflow-y-auto">
                  <div
                    className="text-sm text-white/80 leading-relaxed"
                    dangerouslySetInnerHTML={{
                      __html: formatChangelog(latestRelease.body),
                    }}
                  />
                </div>
              </div>
