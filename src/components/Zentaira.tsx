...
export default function Zentaira() {
  ...
  const [tunnelStatus, setTunnelStatus] = useState<TunnelStatusType | null>(
    null,
  );
  ...
  const getConnectionStatus = () => {
    ...
    if (!tunnelStatus)
      return {
        text: language === "tr" ? "Kontrol ediliyor..." : "Checking...",
        icon: WifiOff,
        color: "text-gray-400",
      };
    ...
    if (!tunnelStatus.is_active) {
      return {
        text: language === "tr" ? "Bağlantı Kesik" : "Disconnected",
        icon: WifiOff,
        color: "text-red-400",
      };
    }
    ...
    if (tunnelStatus.response_time_ms < 300) {
      return {
        text: language === "tr" ? "Mükemmel Bağlantı" : "Excellent Connection",
        icon: Wifi,
        color: "text-green-400",
      };
    } else if (tunnelStatus.response_time_ms < 600) {
      return {
        text: language === "tr" ? "İyi Bağlantı" : "Good Connection",
        icon: Wifi,
        color: "text-yellow-400",
      };
    } else {
      return {
        text: language === "tr" ? "Yavaş Bağlantı" : "Slow Connection",
        icon: Wifi,
                ...
                <p className="text-sm text-blue-200/80">
                  ...
                  {tunnelStatus && tunnelStatus.response_time_ms > 0 && (
                    <span className="ml-2">
                      ...
                      {tunnelStatus.response_time_ms}ms)
                    </span>
                  )}
                </p>
              </div>
