...
export async function searchNominatimTurkey(
  query: string,
): Promise<CityCoordinates[]> {
  try {
...
export async function searchOpenWeatherMap(
  lat: number,
  lon: number,
): Promise<any> {
  try {
...
export async function searchCitiesWithAPIs(query: string): Promise<{
  ...
  const weatherResults = await Promise.all(
    nominatimResults
      .slice(0, 3)
      .map((city) => searchOpenWeatherMap(city.lat, city.lon)),
  );
...
export function searchTurkishCities(query: string): string[] {
  ...
  const allCities = [
    ...new Set([...TURKISH_CITIES, ...MAJOR_CITIES, ...TOURISTIC_CITIES]),
  ];
